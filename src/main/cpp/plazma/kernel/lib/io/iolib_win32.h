//#pragma once
#ifndef PLAZMA_KERNEL_LIB_IO_IOLIB_WIN32_H
#define PLAZMA_KERNEL_LIB_IO_IOLIB_WIN32_H
...
void _WriteConsole(const HANDLE handle, const std::string &buffer) {
    ...
    const std::wstring wbuffer = utf8_to_wstring(buffer);
...
std::string _ReadConsole(const HANDLE handle) {
    ...
    //std::string buffer = wstring_to_utf8(wbuffer);
    //setg(&buffer[0], &buffer[0], &buffer[0] + buffer.size());

    return wstring_to_utf8(wbuffer);
}
...
#endif // PLAZMA_KERNEL_LIB_IO_IOLIB_WIN32_H